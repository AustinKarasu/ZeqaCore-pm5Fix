...
class PracticeUtil{
	...
	public static function itemToArr(Item $item) : array{
		$output = ["name" => $item->getVanillaName(), "count" => $item->getCount()];
	...
	public static function arrToItem(array $input) : ?Item{
		if(!isset($input["name"], $input["count"])){
			return null;
		}
		$item = StringToItemParser::getInstance()->parse($input["name"]) ?? LegacyStringToItemParser::getInstance()->parse($input['name']);
        if(is_null($item)){
            return null;
        }
	...
	public static function teleport(Entity $entity, Vector3 $pos, ?Vector3 $lookAt = null) : void{
        ...
        NetworkBroadcastUtils::broadcastPackets($entity->getViewers(), [MoveActorAbsolutePacket::create($entity->getId(), $entity->getOffsetPosition($pos), $pitch, $yaw, $yaw, (MoveActorAbsolutePacket::FLAG_TELEPORT | ($entity->onGround ? MoveActorAbsolutePacket::FLAG_GROUND : 0)))]);
