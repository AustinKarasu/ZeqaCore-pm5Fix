...
class CombatBot extends GenericHuman{
	...
	private function doSumoBehavior() : void{
		...
		if($this->distanceToTarget < $this->hitRange * 2 && $this->currentTick % $this->swingCooldownTick === 0){
            ...
            NetworkBroadcastUtils::broadcastPackets($this->getWorld()->getPlayers(), [ActorEventPacket::create($this->getId(), ActorEvent::ARM_SWING, 0)]);
		}
		$this->attackTarget();
	...
	private function eatGappleAction(bool $effect) : void{
		...
		if($item instanceof Food){
            [$netId, $netData] = TypeConverter::getInstance()->getItemTranslator()->toNetworkId($item);
            NetworkBroadcastUtils::broadcastPackets($this->getWorld()->getPlayers(), [ActorEventPacket::create($this->getId(), ActorEvent::EATING_ITEM, ($netId << 16) | $netData)]);
			if($effect){
	...
	protected function onDeath() : void{
		...
		if(($session = PlayerManager::getSession($target = PlayerManager::getPlayerExact($this->target))) !== null){
			...
			if($settingInfo?->isBlood()){
                NetworkBroadcastUtils::broadcastPackets([$target], [LevelEventPacket::standardParticle(ParticleIds::REDSTONE, 0, $vec3)]);
			}
		}
	}
