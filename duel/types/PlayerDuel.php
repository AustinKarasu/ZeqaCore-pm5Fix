...
class PlayerDuel{
				...
				if($this->currentTick % 5 === 0){
					switch($this->kit){
						...
						case "BattleRush":
							/** @var DuelArena $arena */
							$arena = ArenaManager::getArena($this->arena);
							if($this->world->getBlock($pos = $player1->getPosition())->getTypeId() === BlockTypeIds::END_PORTAL){
							...
							if($this->world->getBlock($pos = $player2->getPosition())->getTypeId() === BlockTypeIds::END_PORTAL_FRAME && $this->status === self::STATUS_IN_PROGRESS){
								...
								foreach($this->blocksRemover as $hash => $sec){
									if(--$this->blocksRemover[$hash] === 0){
										...
										PracticeUtil::onChunkGenerated($this->world, $x >> 4, $z >> 4, function() use ($vec3, $air){
											if(($block = $this->world->getBlock($vec3))->getTypeId() !== BlockTypeIds::AIR){
	...
	private function setInDuel() : void{
		...
		$player1->setNoClientPredictions();
		$player2->setNoClientPredictions();
			...
			if($this->kit === "Boxing"){
				...
				if($this->matchData[$name]["numHits"] >= 100 && $this->status === self::STATUS_IN_PROGRESS){
					if(($session = PlayerManager::getSession($this->getOpponent($name))) !== null){
                        NetworkBroadcastUtils::broadcastPackets([PlayerManager::getPlayerExact($name)], [LevelSoundEventPacket::create(LevelSoundEvent::HURT, $session->getPlayer()->getPosition(), -1, "minecraft:player", false, false)]);
						$session->onDeath();
					}else{
	...
	public function adaptKitItems(Player $player) : void{
		...
		foreach($items as $slot => $item){
			if(($terracotta = $item->getTypeId() === BlockTypeIds::GLAZED_TERRACOTTA) || $item->getTypeId() === BlockTypeIds::WOOL){
                $bool = ($player->getName() === $this->player1Name);
				#$inv->setItem($slot, ItemFactory::getInstance()->get(($terracotta ? BlockTypeIds::TERRACOTTA : BlockTypeIds::WOOL), ($player->getName() === $this->player1Name) ? 11 : 14, $item->getCount()));
                $inv->setItem($slot, $terracotta ? VanillaBlocks::GLAZED_TERRACOTTA()->asItem() : VanillaBlocks::WOOL()->setColor($bool ? DyeColor::BLUE : DyeColor::RED)->asItem());
			}
		}
	...
	public function tryBreakOrPlaceBlock(Player $player, Block $block, bool $break = true) : bool{
		if($this->isPlayer($player) && $this->isRunning()){
			if($break){
				if($this->kit === "Bridge"){
					if(!in_array($block->getTypeId(), [BlockTypeIds::GLAZED_TERRACOTTA, BlockTypeIds::CONCRETE], true)){
						return false;
					}
					/** @var DuelArena $arena */
					$arena = ArenaManager::getArena($this->arena);
					...
					if(!in_array($block->getTypeId(), [BlockTypeIds::COBBLESTONE, BlockTypeIds::OAK_PLANKS, BlockTypeIds::OBSIDIAN, BlockTypeIds::WATER, BlockTypeIds::LAVA], true)){
						return false;
					}
					/** @var DuelArena $arena */
					$arena = ArenaManager::getArena($this->arena);
					...
					if(!in_array($block->getTypeId(), [BlockTypeIds::WOOL, BlockTypeIds::END_STONE, BlockTypeIds::OAK_PLANKS, BlockTypeIds::BED], true)){
						return false;
					}
					if($block instanceof Bed){
						...
						if($color === "Blue"){
                            ...
                            NetworkBroadcastUtils::broadcastPackets($players, [PlaySoundPacket::create("mob.enderdragon.growl", $pos->getX(), $pos->getY(), $pos->getZ(), 1, 1)]);
						}elseif($color === "Red"){
					...
					if($block->getTypeId() !== BlockTypeIds::WOOL){
						return false;
					}
					/** @var DuelArena $arena */
					$arena = ArenaManager::getArena($this->arena);
					...
					if(!in_array($block->getTypeId(), [BlockTypeIds::SANDSTONE, BlockTypeIds::BED], true)){
						return false;
					}
					if($block instanceof Bed){
						$name = $player->getName();
					...
					if($block->getTypeId() !== BlockTypeIds::SNOW){
						return false;
					}
					/** @var DuelArena $arena */
					$arena = ArenaManager::getArena($this->arena);
					...
					$player->getInventory()->addItem(VanillaItems::SNOWBALL()->setCount(rand(1, 3)));
				}
				if(PracticeCore::REPLAY){
					$this->setBlockAt($block, true);
				}
				...
				if($this->kit === "BuildUHC" && $block->getTypeId() === BlockTypeIds::MOB_HEAD){
					return false;
				}
				if(PracticeCore::REPLAY){
	...
	public function setThrowFor(string|Player $player, ProjectileItem $item) : void{
		if($item->getTypeId() === ItemTypeIds::SPLASH_POTION && isset($this->matchData[$name = $player instanceof Player ? $player->getName() : $player])){
