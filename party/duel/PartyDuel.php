					...
					switch($this->kit){
						...
						case "BattleRush":
							...
							foreach($team1Sessions as $msession){
								...
								if($block->getTypeId() === BlockTypeIds::END_PORTAL_FRAME && $this->status === self::STATUS_IN_PROGRESS){
	...
	public function adaptKitItems(Player $player) : void{
		...
		foreach($items as $slot => $item){
			if(($terracotta = $item->getTypeId() === BlockTypeIds::GLAZED_TERRACOTTA) || $item->getTypeId() === BlockTypeIds::WOOL){
                $color = PartyManager::getPartyFromName($this->party1)->isPlayer($player) ? DyeColor::RED() : DyeColor::BLUE();
                $inv->setItem($slot, $terracotta ? VanillaBlocks::GLAZED_TERRACOTTA()->setColor($color)->asItem() : VanillaBlocks::WOOL()->setColor($color)->asItem());
			}
		}
		...
		if(($team = $this->getTeam($player)) !== null && $this->isRunning()){
			if($break){
				if($this->kit === "Bridge"){
					...
					$player->getInventory()->addItem(VanillaItems::SNOWBALL()->setCount(mt_rand(0,3)));
				}
				$pos = $block->getPosition();
				if(!isset($this->chunks[$hash = World::chunkHash($pos->getFloorX() >> 4, $pos->getFloorZ() >> 4)])){
					$this->chunks[$hash] = new ChunkCache();
