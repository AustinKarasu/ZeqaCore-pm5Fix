...
class CustomEntity{
	...
	public function sendData(array|Player $player, ?array $data = null) : void{
		...
		$pk = SetActorDataPacket::create($this->getId(), $data ?? $this->getSyncedNetworkData(false), new PropertySyncData([], []),0);
		foreach($player as $p){
	...
	public function spawnTo(Player $player) : void{
		...
		$player->getNetworkSession()->sendDataPacket(AddActorPacket::create($this->getId(), $this->getId(), 'custom:entity', $this->location->asVector3(), null, $this->location->pitch, $this->location->yaw, $this->headYaw, [], $this->getSyncedNetworkData(false), [], new PropertySyncData([],[]),[]));
