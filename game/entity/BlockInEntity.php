...
class BlockInEntity extends CustomEntity{
	...
	public function spawnTo(Player $player) : void{
        ...
        $player->getNetworkSession()->sendDataPacket(PlayerListPacket::add([PlayerListEntry::createAdditionEntry($this->uuid, $this->id, $this->getName(), TypeConverter::getInstance()->getSkinAdapter()->toSkinData($this->skin))]));
        $player->getNetworkSession()->sendDataPacket(AddPlayerPacket::create($this->uuid, TextFormat::YELLOW . "Zeqa.net", $this->getId(), (string) $this->getId(), $this->location->asVector3(), null, $this->location->pitch, $this->location->yaw, $this->headYaw, ItemStackWrapper::legacy(TypeConverter::getInstance()->coreItemStackToNet(VanillaItems::AIR())),GameMode::SURVIVAL, $this->getSyncedNetworkData(false), new PropertySyncData([], []), UpdateAbilitiesPacket::create(new AbilitiesData(0, 0, $this->getId(), [])), [], $this->getName(), 1));
	...
	public function despawnFrom(Player $player) : void{
        ...
        $player->getNetworkSession()->sendDataPacket(PlayerListPacket::remove([PlayerListEntry::createAdditionEntry($this->uuid, $this->id, $this->getName(), TypeConverter::getInstance()->getSkinAdapter()->toSkinData($this->skin))]));
	}
